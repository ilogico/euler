...
class Card
	...
	isFollowedBy: (card)->
		if card.rank == @rank + 1
			...
			@rank == 13 and card.rank == 1

poker = (a, i)->
	rank[0+i] == rank[1+i] == rank[2+i] == rank[3+i]
twos = (a, i)->
	rank[0+i] == rank[1+i]
threes = (a, i)->
...
findPokers = (a)->
	if poker(a, 0)
		{combo: 'poker', comp: [a[0].rank, a[5].rank]}
	else if poker(a, 1)
		{combo: 'poker', comp: [a[1].rank, a[0].rank]}
	else if threes(a, 0)
	
		{combo: 'fullen', comp: [a[0].rank, a[0].rank]}
hands = hands.split('\n').map (line) ->
	l = line.split(' ')
	[l[0...5], l[6..]]
console.log hands